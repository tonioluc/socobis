...
public class AdminPrevision {
    ...
    public String getRequete(String datyFiltre, String deb, String fin, String grouper) throws Exception {
        String requette = " SELECT daty, debit, credit,semaine,mois,annee "
                ...
                + "   WHERE DATY >= TO_DATE('" + deb + "','DD/MM/YYYY') AND DATY < TO_DATE('" + datyFiltre
                + "','DD/MM/YYYY') "
                + "   UNION ALL ( SELECT daty, debit, credit,semaine,mois,annee "
                + "                FROM RESULTATPREVISIONNELTOUSMVT "
                + "                     WHERE DATY >= TO_DATE('" + datyFiltre + "','DD/MM/YYYY') and DATY <= TO_DATE('"
                + fin + "','DD/MM/YYYY') ) ";

        if (grouper != null && grouper.compareToIgnoreCase("semaine") == 0) {
            requette = "select min(daty) as daty, sum(debit) as debit, sum(credit) as credit from (" + requette
                    + ") group by semaine,mois,annee";
        }
    ...
    public void getPrevision(String datyFiltre, String deb, String fin, String grouper) throws Exception {
        ...
        try {
            ...
            for (Prevision prevision : previsions) {
                System.out.println(prevision.getDaty() + " | " + prevision.getDebit() + " | " + prevision.getCredit());
                System.out.println(prevision.getId());
            }
            ...
            for (int i = 1; i < this.getListePrev().length; i++) {
                ...
                if (minimum.getSoldeFinale() > listePrev[i].getSoldeFinale())
                    this.setMinimum(listePrev[i]);
            }
        } catch (Exception e) {
            throw e;
        } finally {
