...
public class Vente extends FactureCF {
    ...
    String detailVenteMultiple;

    public String getDetailVenteMultiple() {
        return detailVenteMultiple;
    }

    public void setDetailVenteMultiple(String detailVenteMultiple) {
        this.detailVenteMultiple = detailVenteMultiple;
    }
    ...
    public void payerAvecTTC(String u, Connection c, double montantTtcForce) throws Exception {
        if(this.getEtat() < ConstanteEtat.getEtatValider())
            throw new Exception("Impossible d encaisser une vente non validée");
    ...
    public Object validerObject(String u, Connection c) throws Exception {
        ...
        try {
            ...
            if(this.getEstPrevu() == 1 && this.getDatyPrevu()!=null){
                Vente venteComplet = this.getVenteWithMontant(c);
                String detailleVenteMultiple =this.getDetailVenteMultiple();
                System.out.println("...................................."+ detailleVenteMultiple);
                if(detailleVenteMultiple != null && !detailleVenteMultiple.trim().isEmpty()){
                    Map<Date, Double> dateTauxMap = Vente.parseDateTauxList(detailleVenteMultiple, venteComplet.getMontantttcAr());
...
//            compta[i].setDebit((montantHT-retenue) * ((this.getTva()/100)));
    ...
    public void genererEcritureEncaissementAvecTTC(String u, Connection c, double montantTtc) throws Exception{
        // 1. Création de l'écriture mère
        ComptaEcriture mere = new ComptaEcriture();
        Date dateDuJour = utilitaire.Utilitaire.dateDuJourSql();
        int exercice = utilitaire.Utilitaire.getAnnee(daty);
        ...
        // 2. Génération des sous-écritures avec le montant forcé
        ComptaSousEcriture[] filles = this.genererSousEcritureEncaissementAvecTTC(u, c, montantTtc);
        
        // 3. Enregistrement des lignes
        for(int i=0; i<filles.length; i++){
            filles[i].setIdMere(mere.getId());
            filles[i].setExercice(exercice);
            filles[i].setDaty(this.getDaty());
    ...
    public ComptaSousEcriture[] genererSousEcritureEncaissementAvecTTC(String refUser, Connection c, double montantTtcForce) throws Exception{
        ...
        try{
            ...
            // Récupération des infos vente
            Vente[] ventes = (Vente[]) CGenUtil.rechercher(
                new Vente("VENTE_MERE_MONTANT"), null, null, c, 
                " and id = '"+this.getId()+"'"
            );
            if(ventes.length<1) throw new Exception("Facture mere Introuvable");
            ...
            // DIFFÉRENCE CLEF : on utilise montantTtcForce au lieu de ventes[0].getMontantttc()
            double montant = montantTtcForce;

            compta = new ComptaSousEcriture[2];
            
            // LIGNE 1 : Débit Caisse (entrée d'argent)
            compta[0] = new ComptaSousEcriture();
            compta[0].setLibellePiece(this.getDesignation());
            compta[0].setRemarque(this.getDesignation());
            compta[0].setCompte(getCaisse(c).getCompte());  // Ex: 53100
            compta[0].setDebit(montant);  // ← MONTANT PERSONNALISÉ

            // Mouvement de caisse physique
            MvtCaisse mvt = new MvtCaisse();
            mvt.setCredit(montant);  // Crédit = entrée caisse
            mvt.setIdCaisse(getCaisse(c).getId());
            mvt.setDaty(utilitaire.Utilitaire.dateDuJourSql());
            mvt.setDesignation("mvt pour " + this.getDesignation());
            mvt.setIdOrigine(this.getId());
            ...
            compta[1].setLibellePiece("Encaissement Client "+ventes[0].getClientlib());
            compta[1].setRemarque("Encaissement Client "+ventes[0].getClientlib());
            compta[1].setCompte(this.getCompte());  // Ex: 41100001
            compta[1].setCredit(montant);  // ← MONTANT PERSONNALISÉ
    ...
    public void genererEcritureAnnulationEncaissement(String u, Connection c, double montantReversed) throws Exception{
        // 1. Création de l'écriture comptable mère
        ComptaEcriture mere = new ComptaEcriture();
        Date dateDuJour = utilitaire.Utilitaire.dateDuJourSql();
        int exercice = utilitaire.Utilitaire.getAnnee(daty);
        ...
        mere.setOrigine(this.getId());
        mere.setIdobjet(this.getId());
        mere.createObject(u, c);  // Enregistrement en base
        
        // 2. Génération des sous-écritures (lignes comptables)
        ComptaSousEcriture[] filles = genererSousEcritureAnnulationEncaissement(u, c, montantReversed);
        
        // 3. Liaison et enregistrement de chaque ligne
        for(int i=0; i<filles.length; i++){
            filles[i].setIdMere(mere.getId());  // Lien avec l'écriture mère
            filles[i].setExercice(exercice);
            filles[i].setDaty(this.getDaty());
            filles[i].setJournal(ConstanteStation.JOURNALVENTE);
    ...
    public ComptaSousEcriture[] genererSousEcritureAnnulationEncaissement(String refUser, Connection c, double montantReversed) throws Exception{
        ...
        try{
            ...
            // Création des 2 lignes comptables inversées
            compta = new ComptaSousEcriture[2];
            
            // LIGNE 1 : Crédit Caisse (sortie d'argent)
            compta[0] = new ComptaSousEcriture();
            ...
            compta[0].setCompte(getCaisse(c).getCompte());  // Ex: 53100
            compta[0].setCredit(montantReversed);  // ← CRÉDIT (sortie)
            
            // LIGNE 2 : Débit Client (on lui re-doit)
            compta[1] = new ComptaSousEcriture();
            ...
            compta[1].setCompte(this.getCompte());  // Ex: 41100001 (compte client)
            compta[1].setDebit(montantReversed);   // ← DÉBIT (on lui redoit)
...
//        valeur.setIdAvoirFC(venteDetails.get());
    ...
    public void annulerVenteCustom(String u, Connection c, String idMagasinAnnulation, double totalReversed) throws Exception {
        inverserMouvementsStock(u, c, idMagasinAnnulation);
        annulerEncaissementsLies(u, c, totalReversed);
    }
    ...
    private void annulerEncaissementsLies(String u, Connection c, double totalReversed) throws Exception {
        if(totalReversed > 0){
            MvtCaisse mvt = new MvtCaisse();
