...
<%@ page import="org.bson.util.ClassMap" %>
...
<%
    ...
    if (acte.compareToIgnoreCase("updateMvt") == 0) {
        ...
        id = f.getTuppleID();
        try{
                if("caisse.MvtCaisse".equalsIgnoreCase(classe)){
                    String idPrev = request.getParameter("idPrevision");
                    String debitStr = request.getParameter("debit");
                    if(idPrev!=null && !idPrev.isEmpty() && debitStr!=null && !debitStr.isEmpty()){
                        prevision.PrevisionComplet prev = new prevision.PrevisionComplet();
                        prev.setNomTable("MVTCAISSEPREVISION");
                        prev.setId(idPrev);
                        prev = (prevision.PrevisionComplet) prev.getById(idPrev,"PREVISION_COMPLET_CPL",null);
                        caisse.MvtCaisse mvt = (caisse.MvtCaisse) f;
                        prevision.MvtCaissePrevision lien = new prevision.MvtCaissePrevision();
                        System.out.println("lululuuuuu");
                        lien.setId1(id);
                        lien.setId2(idPrev);
                        lien.setTaux(mvt.getTaux());
                        lien.setDevise(mvt.getIdDevise());
                        lien.setEtat(ConstanteEtat.getEtatValider());
                        double montantBrut = utilitaire.Utilitaire.stringToDouble(debitStr);
                        lien.setMontant(montantBrut, prev);
                        lien.setEtat(mvt.getEtat());
                        u.createObject(lien);
    ...
    if (acte.compareToIgnoreCase("valider") == 0) {     // VISER
        ...
        if (o instanceof VenteRetourClient) {
            ((VenteRetourClient) o).echanger(u.getUser().getTuppleID(), null);
        }
        temp = t;
        val = o.getTuppleID();
...
<%
    ...
    if (acte.compareToIgnoreCase("validerEtPayer") == 0) {
        System.out.println("Tsy aiko hoe mety ve le izy");
        t = (ClassMAPTable) (Class.forName(classe).newInstance());
        t.setValChamp(t.getAttributIDName(), request.getParameter("id"));
        if (nomtable != null && nomtable.compareToIgnoreCase("") != 0) t.setNomTable(nomtable);

        ClassMAPTable op = (ClassMAPTable) u.validerObject(t);
        InsertionVente iv = null;
        if ("vente.VenteRetourClient".equals(classe) && op instanceof VenteRetourClient) {
            iv = ((VenteRetourClient) op).echangerEtPayer(u.getUser().getTuppleID(), null);
            System.out.println("Efa tonga ato ve");
            System.out.println("Iv dia " + iv);
            if (iv != null) {
                System.out.println("Ty le id vente ee" + iv.getId());
            }
        }
        temp = t;
        val = op.getTuppleID();
    ...
    <script language="JavaScript">
        <% if ("vente.VenteRetourClient".equals(classe) && op instanceof VenteRetourClient){ %>
        document.location.replace("<%=lien%>?but=<%=bute + rajoutLie%>&id=<%=val%>&idVente=<%=iv != null ? iv.getId() : ""%>");
        <% } else { %>
        document.location.replace("<%=lien%>?but=<%=bute + rajoutLie%>&id=<%=val%>");
        <% } %>
    </script>

    <%
    }
    if (acte.compareToIgnoreCase("validerPayerEtLivrer") == 0) {
        System.out.println("Ty le vaovao");
        t = (ClassMAPTable) (Class.forName(classe).newInstance());
        t.setValChamp(t.getAttributIDName(), request.getParameter("id"));
        if (nomtable != null && nomtable.compareToIgnoreCase("") != 0) t.setNomTable(nomtable);
        ...
        InsertionVente iv = null;
        if ("vente.VenteRetourClient".equals(classe) && op instanceof VenteRetourClient) {
            iv = ((VenteRetourClient) op).echangerEtPayerEtLivrer(u.getUser().getTuppleID(), null);
            System.out.println("Efa tonga ato ve");
            System.out.println("Iv dia " + iv);
            if (iv != null) {
                System.out.println("Ty le id vente ee" + iv.getId());
    ...
    if (acte.compareToIgnoreCase("validerMvt") == 0) {     // VISER
        t = (ClassMAPTable) (Class.forName(classe).newInstance());
        t.setValChamp(t.getAttributIDName(), request.getParameter("id"));
        if (nomtable != null && nomtable.compareToIgnoreCase("") != 0) t.setNomTable(nomtable);
        MvtCaisse caisse = (MvtCaisse) t;
        ClassMAPTable o = caisse.validerSimple(u.getUser().getTuppleID(), null);
        temp = t;
        val = o.getTuppleID();
%>
<script language="JavaScript"> document.location.replace("<%=lien%>?but=<%=bute + rajoutLie%>&id=<%=val%>");</script>
