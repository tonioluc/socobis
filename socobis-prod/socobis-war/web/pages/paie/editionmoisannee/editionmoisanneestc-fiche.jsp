...
<%
    try {
        ...
        String butefiche = "paie/editionmoisannee/editionmoisanneestc-fiche.jsp";
        String lien = (String) session.getValue("lien");

        UserEJB u = (UserEJB) session.getAttribute("u");
        
        paie = new PaieEditionmoisanneeLib();
        paie.setNomTable("PAIE_EDITIONMOISANNEESTC_LIB_2");

        PageConsulte pc = new PageConsulte(paie, request, u);
        pc.setTitre("Fiche paie edition mois ann&eacute;e ");
        ...
        <div class="col-md-6">
            <div class="box-fiche">
                <div class="box">
                    ...
                    <div class="row">
                        <div class="col-md-8 offset-md-2 box-footer">
                            <div class="button-group">
                                ...
                                <%--                                <form action="/socobis/EtatReportJasper?action=fiche_de_paie" method="post" name="heuresuppl" id="heuresuppl" style="display: inline-block;">--%>
                                <%--                                    <input type="hidden" name="mois" value="<%= paie.getMois() %>">--%>
                                <%--                                    <input type="hidden" name="annee" value="<%= paie.getAnnee() %>">--%>
                                <%--                                    <input type="hidden" name="etat" value="<%= paie.getEtat() %>">--%>
                                <%--                                    <input type="hidden" name="id" value="tous">--%>
                                <%--                                    <input type="hidden" name="edition" value="<%= paie.getId() %>">--%>
                                <%--                                    <button type="submit"--%>
                                <%--                                            class="btn btn-info"--%>
                                <%--                                            name="bulletinglob"--%>
                                <%--                                            aria-label="G&eacute;n&eacute;rer le bulletin de paie"--%>
                                <%--                                            title="Bulletin de Paie">Bulletin de Paie</button>--%>
                                <%--                                </form>--%>

                                <button class="btn btn-success" id="exportAllBtn">Voir bulletin de paie</button>
                            </div>
                        </div>
                    </div>
                </div>
...
<%
    try {
        ...
        List<FichePaie> allFichePaies = fp.getListFichePaieEditionStc(id);
%>
<script>
    ...
    function createPayslipHTML(employee) {
        ...
        return '' +
            ...
            '<img src="' + contextPath + '/assets/img/xc-factory-logo.jpeg" alt="XC Factory Logo" onerror="this.style.display=\'none\'; this.nextElementSibling.style.display=\'block\';">' +
            '<div class="fallback-text">XC Factory</div>' +
            '</div>' +
            '<div class="company-address">' +
            'T&eacute;l.: 034 42 276 10<br>' +
            'Zone d\'Activit&eacute;s THUYA MAXIMUS1 Andohatapenaka<br>' +
            '101 - ANTANANARIVO' +
            '</div>' +
            '</div>' +
            ...
            '<tr><td><label>Cat&eacute;gorie professionnelle :</label></td><td>' + employee.fonction + '</td></tr>' +
            '</table>' +
            '</div>' +
            '</div>' +
            ...
            '<tr class="data-row"><td>Cong&eacute;s pay&eacute; :</td><td></td><td>-</td></tr>' +
            ...
            '<tr class="data-row"><td>Heures Suppl&eacute;mentaires : <span>+jours f&eacute;ri&eacute;s</span></td><td></td><td></td></tr>' +
            ...
            '</tr>' +
            '<tr class="data-row"><td>Avance : <span>(Quinzaine & Sp&eacute;ciale)</span></td><td></td><td>-</td></tr>' +
            ...
            '<img style="width: 75px; height: 45px" src="' + contextPath + '/assets/img/xc-factory-logo.jpeg" alt="XC Factory Logo" class="footer-logo" onerror="this.style.display=\'none\'; this.nextElementSibling.style.display=\'block\';">' +
            '<div class="logo" style="display: none;">XC Factory</div>' +
            '</div>' +
